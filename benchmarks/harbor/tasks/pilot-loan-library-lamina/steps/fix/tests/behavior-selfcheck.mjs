...
export async function runBehaviorSelfcheck({ root = '/app', golden }) {
  ...
  for (const [, prefix] of prefixes) {
    async function reducePrefix() {
      let s = await resolve(mod.createInitialState());
      for (const action of prefix) {
        s = await resolve(mod.reduce(s, action));
      }
      return s;
    }
    const actor = prefix[prefix.length - 1].actor || 'owner';
    const viewA = snap(await resolve(mod.project(await reducePrefix(), actor)));
    const viewB = snap(await resolve(mod.project(await reducePrefix(), actor)));
    if (viewA !== viewB) {
      errors.push(`${prefix[prefix.length - 1].type}: project() is not a pure function of reduced state`);
      break;
    }
  }
