# control-simple-list — lamina init
...
Run **only** `/lamina-init` via the `lamina-init` skill (full establish mode). Write real `business-context.md` + evidence-grounded `personas.json` (≥2 materially distinct personas for multi-actor briefs). Do not run `/lamina-design`, `/lamina-verify`, or implement application code in this step.

## Lamina bench profile (required)

- Contract stage: start from **`spark`**. Follow `/lamina-design` stage rules — apply harden-level rigor at in-scope authority, privacy, and lifecycle boundaries without expanding into production auth/infra.
