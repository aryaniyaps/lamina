...
const rows = [
  ...
  row('small.semantic.02-route', ['small.route-completion'], ['small.surface.route-change-complete'],
    'The route query belongs to the Workflow that handles completion of a reviewed dashboard route change.'),
  ...
  row('small.semantic.07-role_boundary', ['small.discussion-authorization', 'small.dashboard-navigation'],
    ...
    'A dashboard discussion action can plausibly mean authorizing the action or navigating to it, so both exact alternatives are retained and neither is selected.'),
  ...
  row('small.semantic.10-entry_point', ['small.dashboard-navigation'], ['small.surface.side-navigation-item'],
    'The visible side-navigation item is the reviewed entry surface for the Dashboard navigation Workflow.'),
  row('small.semantic.11-command', ['small.disclosure-state'], ['small.surface.use-disclosure'],
    'The disclosure operation supplies command semantics for the Workflow; the hook is its localization surface, not a repository package-script claim.'),
  ...
  row('medium.semantic.06-permission', ['medium.api-key-actions', 'medium.oidc-discovery-test'],
    ...
    'A developer-authentication setup request can plausibly refer to API-key actions or OIDC discovery verification, so both exact alternatives remain unselected.'),
  ...
  row('medium.semantic.13-route', ['medium.oidc-discovery-test'], ['medium.surface.oidc-body-type'],
    'OIDC discovery verification is the public-seed Workflow whose reviewed response-contract test is relevant to the discovery route.'),
  ...
  row('large.semantic.03-role_boundary', ['large.ai-configuration-form', 'large.feature-preview-deployment'],
    ...
    'An instance-feature preview request can plausibly mean configuring AI fields or deploying a feature preview, which have distinct administrator and maintainer authority.'),
  ...
  row('large.semantic.06-entry_point', ['large.authentication-error-family'], ['large.surface.auth-error-handler'],
    'The administrator authErrorHandler is the reviewed entry surface for handling the corresponding authentication failure.'),
  row('large.semantic.07-command', ['large.feature-preview-deployment'], ['large.surface.feature-preview-chart'],
    'The invocable feature-deployment Workflow and its reviewed Helm variable surface are relevant to the deployment command.'),
  ...
  row('large.semantic.10-flag', ['large.feature-preview-deployment'], ['large.surface.feature-preview-chart'],
    'The reviewed feature-preview Helm variable is the Workflow-relevant feature control for preview deployment.'),
  ...
  row('large.semantic.12-event', ['large.feature-preview-deployment'], ['large.surface.feature-preview-chart'],
    'The approved maintainer invocation is the public-seed trigger semantics; the Helm variable localizes the Workflow but is not claimed as an emitted runtime event.'),
  ...
  row('large.semantic.14-permission', ['large.ai-configuration-form', 'large.feature-preview-deployment'],
    ['large.surface.ai-form-values', 'large.surface.feature-preview-chart'],
    'The linked-worktree permission boundary coordinates two privileged instance-feature Workflows while preserving each public-seed actor authority.'),
];
...
export const SEMANTIC_CASE_MAPPING_CANONICAL_SHA256 = 'f773a7ba3d9e62b6c431546bf3d49ce46c523beaaae8c7505d4769342e740597';

export function validateSemanticCaseMapping(value) {
  const errors = [];
  if (value?.schema !== 'lamina.real-repository-oracle-semantic-case-mapping/v1' || value.version !== 1
  ...
  for (const item of value.rows) {
    if (!/^(small|medium|large)\.semantic\.\d{2}-[a-z_]+$/.test(item.id)
      ...
      || ![null, 'feature_flags'].includes(item.separate_lexical_category)) {
  ...
  const separateRows = value.rows.filter((item) => item.separate_lexical_category !== null);
  if (separateRows.length !== 1 || separateRows[0]?.id !== 'medium.semantic.08-flag'
    || separateRows[0]?.separate_lexical_category !== 'feature_flags') {
    errors.push('only medium.semantic.08-flag may bind a separately requested lexical witness');
  }
