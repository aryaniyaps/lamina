...
function validatePackageManifest(manifest) {
  if (!fs.existsSync(packageManifestPath)) {
    errors.push('missing package.manifest-v3.json — run build-transactional-pilot.mjs first');
    return;
  }
...
function validateTaskDir(task, arm) {
  ...
  for (const step of steps) {
    ...
    if (step === finalStep) {
      ...
      !/"reward": 1\.0/.test(testSh) &&
      !/reward.: 1/.test(testSh) &&
      !/node \/tests\/grade\.mjs/.test(testSh)
    ) {
      errors.push(`${dir}: intermediate step ${step} must emit pass-through reward or run the structural grader`);
    }
  }
  ...
  if (arm === 'lamina') {
    ...
    if (!/init-graph\.json/.test(init) || !/lamina graph backup/.test(init)) {
      errors.push(`${dir}: lamina_init must export graph-backed evaluator evidence`);
    }
    if (!/design-graph\.json/.test(design) || !/\.lamina\/projections\/implement\.md/.test(design)) {
      errors.push(`${dir}: lamina_design must export a version-pinned graph projection and backup`);
    }
    for (const step of steps) {
      const testsDir = path.join(dir, 'steps', step, 'tests');
      if (!fs.existsSync(path.join(testsDir, 'pilot-treatment.mjs'))) {
        errors.push(`${dir}: ${step} must package the fail-closed Lamina treatment gate`);
      }
    }
  } else {
    if (arm === 'direct') {
