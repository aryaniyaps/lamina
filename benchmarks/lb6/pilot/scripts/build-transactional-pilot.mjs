...
const armPrompts = {
  ...
  lamina:
    'Use the installed `$lamina` skill and its contained `/lamina-*` routes fully. Follow Mode B: during `/lamina-*` commands write only under `.lamina/`; implement application source in separate coding turns. ' +
...
function dockerfile(arm = 'direct', cliBinarySha256 = null, cliVersion = null) {
  const laminaRuntime = arm === 'lamina'
    ...
    : '';
  const laminaRepository = arm === 'lamina'
    ? (
        'RUN git init -b main \\\n' +
        '    && git config user.email benchmark@lamina.invalid \\\n' +
        '    && git config user.name "Lamina Benchmark" \\\n' +
        '    && git commit --allow-empty -m "benchmark fixture" \\\n' +
        '    && mkdir -p .lamina/runtime \\\n' +
        '    && ln -s ../.lamina/runtime .git/lamina\n'
      )
    : '';
  return (
...
function laminaPersonaBlock() {
  return (
    ...
    'Each child must run on requested `composer-2.5`. Preserve child `agentId`, success, duration, and conversation steps in the parent transcript; publish only normalized simulated evidence through graphd.\n\n' +
...
function laminaStepCommand(phase) {
  const commands = {
    lamina_init:
      'Route through the installed `$lamina` skill and run **only** `/lamina-init`. ' +
      'Write real `business-context.md` + evidence-grounded `personas.json` (≥2 materially distinct personas). ' +
      'Observe those sources, publish canonical Product, Persona, and Actor Resources through an explicit graph session, then export evaluator evidence with ' +
      '`mkdir -p .lamina/benchmark && lamina graph backup --output .lamina/benchmark/init-graph.json`. ' +
      'Do not implement application code in this step.',
    lamina_design:
      'Route through the installed `$lamina` skill and run **only** `/lamina-design` through a validated, atomically published GraphVersion. ' +
      ...
      'Create `.lamina/projections/implement.md` strictly from the published graph query and cite its exact GraphVersion plus source revision. ' +
      'The projection must specify reducer-enforced authority, illegal-state bans, and edge/recovery paths the next coding step must ship in `app.mjs` (not UI-only). ' +
      'Finally export evaluator evidence with `mkdir -p .lamina/benchmark && lamina graph backup --output .lamina/benchmark/design-graph.json`.',
    implement:
      'Implement the **published ABI** (`app.mjs` / `ui.mjs`) from `.lamina/projections/implement.md` in a normal coding turn. ' +
...
if (arm === 'lamina') {
  ...
  else if (${JSON.stringify(['lamina_init', 'lamina_design'].includes(phase))}) reward = 1;
}
...
function finalTreatmentGateSource(phase) {
  return `#!/usr/bin/env node
import fs from 'node:fs';
import { checkPilotLaminaTreatment } from './pilot-treatment.mjs';
...
function writeTask(task, arm, ctx) {
  ...
  for (const step of steps) {
    ...
    const isFinal = step.name === finalStep;
    if (isFinal) {
      const testFinal = arm === 'lamina'
        ? templates.testFinal.replace(
            'mkdir -p /logs/verifier\n',
            'mkdir -p /logs/verifier\nnode /tests/treatment-gate.mjs\n',
          )
        : templates.testFinal;
      fs.writeFileSync(path.join(stepDir, 'test.sh'), testFinal);
      ...
      if (arm === 'lamina') {
        fs.writeFileSync(path.join(stepDir, 'treatment-gate.mjs'), finalTreatmentGateSource(step.name));
        fs.writeFileSync(path.join(stepDir, 'pilot-treatment.mjs'), pilotTreatment);
      }
    } else if (arm === 'lamina') {
      fs.writeFileSync(path.join(stepDir, 'test.sh'), '#!/usr/bin/env bash\nset -euo pipefail\nnode /tests/grade.mjs\n');
      fs.writeFileSync(path.join(stepDir, 'grade.mjs'), structuralGradeSource(task, arm, step.name));
      fs.writeFileSync(path.join(stepDir, 'pilot-treatment.mjs'), pilotTreatment);
...
export function buildPilot({
  ...
  fs.writeFileSync(path.join(ctx.runtimeRoot, 'Dockerfile'), writeTextFile('', dockerfile('direct')));
        ...
        JSON.stringify(
          {
            ...
            benchmark_version: BENCHMARK_VERSION,
            campaign_id: SKILL_RERUN_CAMPAIGN_ID,
            pinned_skill_commit: sourceSkillCommit,
