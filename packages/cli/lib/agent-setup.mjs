...
4. Create a \`lamina.work-map/v2\` mapping every obligation and every compiled Experience Case to current evidence, intended code targets, fixtures, steps, expected observations, and verification. Run \`lamina work check --packet <packet.json> --map <work-map.json>\`.
5. Implement the mapped work.
6. Run the one-shot \`lamina graph observe\` (never foreground \`--live\`, which is a persistent operator watcher), compile and run every relevant graph-backed Persona Mission against the built product, then publish each Run session. Rebase a later Run session before publishing when an earlier independent Run advanced the branch. Every compiled Experience Case needs a passing case-bound Experience Evidence manifest. For UI surfaces, emit all four live audit classes with distinct real artifacts scoped to a Mission surface and concrete state.
7. Update the map with passing artifact paths and run \`lamina work verify\`. Fix failures and repeat until it passes.

Treat the exact graph closure and its Experience Contract as product authority. Source retrieval only localizes code; it cannot override the graph. A user-facing workflow is not implementation-ready until inputs, requiredness, relationship identity/cardinality, duplicate and self-reference behavior, visible success/failure/recovery, surface states, and invariant probes compile into deterministic Experience Cases. For UI work, verification must include functional, visual, responsive, and accessibility evidence. Missing browser or audit capability blocks verification.
