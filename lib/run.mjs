...
/** @typedef {'product' | 'contract'} FindingFixTarget */
/** @typedef {{ id: string; priority: string; finding: string; impact?: string; effort?: string; recommendation?: string; screen_id?: string; flow_id?: string; fix_target?: FindingFixTarget; severity?: string; description?: string; category?: string; screen?: string; workflow_ref?: string; invariant_ref?: string; scenario_ref?: string; status?: string }} FindingItem */
...
const FINDING_FIX_TARGET = new Set(['product', 'contract']);
...
export function validateRunFields(run, rel = 'run.yaml') {
  ...
  for (const f of /** @type {FindingItem[]} */ (run.findings ?? [])) {
    ...
    const findingText = f.finding || f.description;
    if (!findingText) errors.push(`${rel}: finding "${f.id || '?'}" missing finding or description text`);
