...
From `../../references/problem-router.md` (or orchestrator context), list **1–4
Lamina topic references** the main thread should load next, with one line each
on why. Name the compact capability and exact reference path.
...
Expected behavior: load `lamina-research` with
`references/research-synthesis.md`, read all interview files and pasted tickets,
return the output template with cited problems/signals, and recommend exact
compact capability references such as user modeling or problem framing.
