...
12. Write ticket-shaped `findings[]`: `fix_target`, `priority`, `summary`/`finding`, `evidence`, `acceptance` (for product|contract). Classify CI/deploy/push as `ops` or omit. Empty `findings[]` is allowed only when probes passed, every `status: new` screen is observed, **build integrity is clean**, and brief-named flows/edges that belong in product are observed — still emit `fix.md`.
