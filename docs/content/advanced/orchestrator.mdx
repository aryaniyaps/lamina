...
One subagent per `primary: true` actor in `personas.yaml`. Each walks the live app independently, checking their workflows and permissions.
...
`merge-rules.md` defines how skill outputs combine into `run.yaml` — grounding requirements, conflict resolution via `decisions.md`, and incremental writes.
...
- [Skill map](/docs/reference/skill-map) — all capability skills
- [Validation](/docs/reference/validation) — validate contracts
- [LaminaBench](/docs/advanced/lamina-bench) — benchmark methodology
