...
Pilot matrix: `pilot-care-circle` + `control-simple-list` × `{direct, checklist, lamina}` × 2 attempts on Sonnet.
...
- Confirmatory cohort on `opus` only after Sonnet pilot shows valid treatment + directional lift
