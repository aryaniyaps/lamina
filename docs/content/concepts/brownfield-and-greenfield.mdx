...
2. `/lamina-design <flow>` — full contract with `screens[].status: new`
...
All screens in `run.yaml` are marked `new` — nothing exists yet.
...
2. Option A: `/lamina-design` — contract references existing screens via `screens[].source`
...
<Callout type="info">
**Advanced:** Brownfield references are read-only. Lamina cites repo paths in `run.yaml` but never edits app source during Lamina commands.
</Callout>
...
See [What to pair](/docs/guides/pair-with) for integration guidance.
...
- [/lamina-verify](/docs/commands/lamina-verify) — verify existing or new builds
- [What to pair](/docs/guides/pair-with) — tools for brownfield projects
