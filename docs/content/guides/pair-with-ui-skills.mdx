...
```text
1. /lamina-init <domain>        → domain charter
2. /lamina-design <flow>        → validated GraphVersion + projection
3. Your agent implements         → app source from resolved version
4. /lamina-verify guest checkout at http://localhost:3000 → fix behavior gaps
5. UI skill pass                 → polish interface
6. /lamina-verify guest checkout at http://localhost:3000 → confirm behavior still holds
```
...
2. If findings appear, fix behavior from the version-cited finding projection
...
- [Pair with coding workflows](/guides/pair-with) — broader pairing guide
- [HavenStay historical evidence](/guides/demo-havenstay) — an older behavior comparison
