...
3. Check `run.yaml` status is `ready_to_build`
...
2. **Defines actors** — roles, goals, permissions (references `personas.yaml`)
...
<Callout type="info">
**Advanced:** Design uses audit profiles to route sections to capability skills — invariants, dependencies, error handling, accessibility, and more. See [Orchestrator](/docs/advanced/orchestrator).
</Callout>
...
| File | Content |
|------|---------|
| `run.yaml` | Machine contract (`status: designing` → `ready_to_build`) |
| `implement.md` | Full build handoff for your coding agent |
| `report.md` | Design narrative and rationale |

## Brownfield design

On existing codebases, Lamina performs a bounded repo scan to ground the contract in what already exists. Existing screens are marked `status: existing` with repo `source` paths in `screens[]`.
...
- [run.yaml schema](/docs/reference/run-yaml-schema) — full contract reference
- [/lamina-verify](/docs/commands/lamina-verify) — verify after implementation
