...
Answer the business questions product design depends on, record evidence inputs in `.lamina/`, and publish Product, Actor, and Persona knowledge through `graphd`. Run once per project, or again when the business use case changes.
...
These files are evidence inputs, not canonical graph state. See [Evidence inputs](/reference/global-artifacts).
...
<Callout type="info">
**Advanced:** Brownfield initialization runs `lamina graph discover --brownfield` for explicit CocoIndex coverage and limitations. Live UI evidence is collected separately through Missions.
</Callout>
...
- [Evidence inputs](/reference/global-artifacts) — authority of `business-context.md` and `personas.json`
