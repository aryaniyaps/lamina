...
See [run.json schema](/reference/run-json-schema).
...
A product behavior gap discovered during `/lamina-verify`. Recorded in `findings[]` on `run.json` and summarized in `fix.md` with expected vs observed behavior.
...
- [run.json schema](/reference/run-json-schema) — complete field reference
