...
Lamina installs as one public skill containing 58 workflow and craft modules
organized into three taxonomies. The orchestrator routes design and verify
sections to the right contained module via [audit profiles](/advanced/orchestrator).

## Quick path

- **Focused question?** `/lamina <question>` routes to one contained module (direct mode)
- **Full workflow?** `/lamina-init` first, then `/lamina-design` or `/lamina-verify` loads multiple modules via audit profiles
- **Deep dive?** The modules live inside the [single Lamina skill](https://github.com/aryaniyaps/lamina/tree/main/skills/lamina/skills)
...
<Callout type="info">
**Advanced:** Module files live at
`skills/lamina/skills/lamina-<name>/SKILL.md`. The installer discovers only
`skills/lamina/SKILL.md` by default; do not use `--full-depth`, which would
incorrectly expose internal modules as independent public skills.
</Callout>
