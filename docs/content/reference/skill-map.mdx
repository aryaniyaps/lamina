...
Lamina installs as 59 public sibling skills: the `lamina` command router plus
58 focused workflow and craft skills organized into three taxonomies. The
orchestrator routes design and verify sections to the right focused skills via
[audit profiles](/advanced/orchestrator).

## Quick path

- **Focused question?** `/lamina <question>` routes to one focused skill (direct mode)
- **Full workflow?** `/lamina-init` first, then `/lamina-design` or `/lamina-verify` loads multiple sibling skills via audit profiles
- **Deep dive?** Every skill lives directly under the repository's [skills directory](https://github.com/aryaniyaps/lamina/tree/main/skills)
...
<Callout type="info">
**Advanced:** Focused skill files live at
`skills/lamina-<name>/SKILL.md`. Install with `--skill '*'` so every
cross-referenced sibling is available in the same agent session.
</Callout>
