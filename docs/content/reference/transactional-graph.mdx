...
`lamina graph observe` runs the pinned CocoIndex v1 App shipped in the installed npm package. Each file produces an explicit Observation envelope with snapshot, content hash, locator, extractor identity/version, and normalized payload. A custom target sends authenticated idempotent upsert/delete batches over the local transport. CocoIndex never opens Ladybug.
