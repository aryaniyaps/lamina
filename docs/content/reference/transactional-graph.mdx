...
direct provenance, and stable obligation ids. A derived lexical ranker adds
code-localization candidates, but source search is never graph authority.
Dense retrieval is currently an optional unavailable tier and reports
`lexical_degraded`; exact traversal supplies the decision contract.
