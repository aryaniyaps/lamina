...
`work prepare` is a read-only compiler over the selected workflow closure. It
returns the relevant actors, authority, ordered operations, invariants,
scenarios, surfaces, proofs, direct provenance, and stable obligation ids. A
derived lexical ranker adds code-localization candidates, but source search is
never graph authority. Dense retrieval is currently an optional unavailable
tier and reports `lexical_degraded`; exact traversal already supplies the
decision contract.

Preparation fails closed for stale source identity, contradictions, missing
authority, incomplete operation contracts, missing invariants/scenarios, or a
UI surface without a proof specification. `work check` then requires exactly
one code/test mapping for every obligation before edits.
...
For UI Runs, `verified` requires independent functional, visual, responsive,
